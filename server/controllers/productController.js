...
export const removeFavouritedBy = asyncHandler(async (req, res, next) => {
  ...
  if (product && user) {
    if (
      product.favouritedBy.filter(fave => fave.user.toString() === req.user._id.toString())
        .length === 0
    ) {
      res.status(400);
...
export const createUserBidding = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  await Bidding.findByIdAndUpdate(id, {
    $push: {
      biddings: { ...req.body, date: new Date(), user: req.user },
    },
  });
